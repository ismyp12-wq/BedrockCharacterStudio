/* Studio v5 loader — unified workflow and verified build output */
(async function(){
  if(window.__lyStudioV4Loader)return;
  window.__lyStudioV4Loader=true;

  document.title="LY Bedrock Character Studio v5";
  const version="5.0.0";
  ...
  try{
    ...
    const safeBuild=await read("app-build-safe-v5.js");
    new Function(`${safeBuild}\n//# sourceURL=app-build-safe-v5.js`)();

    const title=document.querySelector(".topbar h1");
    if(title)title.textContent="Bedrock Character Studio v5";
    const eyebrow=document.querySelector(".topbar .eyebrow");
    if(eyebrow)eyebrow.textContent="LY VERIFIED ADD-ON WORKSPACE";
  }catch(error){
    console.error("Studio v5 failed to start",error);
    ...
    if(main&&!document.getElementById("v4LoadError")){
      ...
      box.textContent=`เปิด Studio v5 ไม่สำเร็จ: ${error.message}`;
      main.prepend(box);
    }
  }
})();